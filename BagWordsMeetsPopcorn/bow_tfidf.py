...
class DataLoader(object):
    ...
    def save_meta(self):
        targetfile = 'vsm/{}_meta.csv'.format(self._colname)
...
def build_dictionary():
    ...
    dictionary.save('vsm/dictionary.dict')  # store the dictionary, for future reference
...
def clean_dict_save():
    ...
    clean_dict.save('vsm/dictionary.dict')
    # sort by decreasing doc-frequency
    clean_dict.save_as_text("vsm/dictionary.txt", sort_by_word=False)
...
def build_bow_save():
    ...
    for colname in colnames:
        ...
        target_file = "vsm/{}.bow".format(colname)
...
def build_tfidf_save():
    ...
    for colname in colnames:
        ...
        bow_stream = corpora.MmCorpus('vsm/{}.bow'.format(colname))
        tfidf_stream = model[bow_stream]
        corpora.MmCorpus.serialize('vsm/{}.tfidf'.format(colname), tfidf_stream)
